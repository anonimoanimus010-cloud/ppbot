...
user_premium = {}
pending_tokens = {}
user_emails = {} # Memoria per tenere traccia dell'email attiva
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
   if context.args:
       ...
       if token in pending_tokens:
           ...
           user_lang[user_id] = user_lang.get(user_id, "en")
           
           # Salviamo l'email generata associandola al tuo ID utente
           ...
           await update.message.reply_text(
               t(user_id, "unlock_ok").format(email=email),
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(gestisci_sms, pattern="^sms$"))
